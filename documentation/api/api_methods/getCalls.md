# API коллтреккинг (выгрузка звонков)  
## Содержание  
...
***{{version}}*** - версия API. Может иметь значения *v1*, *v2*. Актуальная версия в настоящее время - это *v2*, в запросах следует указывать именно ее.  
...
**call** - метод, с помощью которого можно получить все звонки принадлежащие непосредственно пользователю, не учитывая звонки к которым он имеет доступ с помощью выданной роли.  
**getByRole** - метод, с помощью которого можно получить звонки к которым пользователь имеет доступ по выданное роли, звонки принадлежащие непосредственно пользователю в данном параметре учитываться не будут.  
**getWithRole** - метод, с помощью которого можно получить все звонки, как принадлежащие непосредственно пользователю, так и выданные ему по роли.  
#### Пример запроса  
...
https://ckct.ru/api/index.php?json/v3/x9xx99xx99xxxx99/call/getByRole/0/2020-05-01/2020-05-02  
...
*totalPages* - всего страниц в ответе (INT)  
*totalCalls* - всего звонков (INT)  
...
*utmSource* – рекламная метка utm_source (STRING)  
*utmMedium* – рекламная метка utm_medium (STRING)  
*utmCampaign* – рекламная метка utm_campaign (STRING)  
*utmContent* – рекламная метка utm_content (STRING)  
*utmTerm* - рекламная метка utm_term (STRING)  
