...
Для отправки событий необходимо выбрать цель которая уже создана в личном кабинете Яндекс.Метрики, если цели нет, то её необходимо создать:

Инструкция по настройке целей на стороне Яндкс.Метрики : https://github.com/yuferev/callkeeper/blob/master/documentation/callkeeper_events/events-Metrika/README.md

Данная цель является не обязательным функционалом, но при её настройке будет отправляться информация в счетчик Яндекс.Метрики по событиям: открытие виджета, переход на сайт, заказ звонка, закрытие виджета и т.д

![Рис.7](images/1234.jpg)
