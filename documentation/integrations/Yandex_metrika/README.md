...
Для более детального формирования отчета можно использовать события которые будут передаваться в цели Яндекс.Метрики:
...
Только входящие звонки - звонки которые были инициированы самим пользователем.

**Длительность:**

Параметр который задается в ручную. После прохождения заданной длительности событие отправляется в цель.

Длительность задается в секундах.
...
Необходимо выбрать в какую цель будут отправляться события. Цель необходимо настроить в Яндекс.Метрике. Подробная инструкция по настройке целей : https://github.com/yuferev/callkeeper/blob/master/documentation/callkeeper_events/events-Metrika/README.md
